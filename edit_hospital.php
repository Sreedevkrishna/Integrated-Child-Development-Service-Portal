...
$cid=$_GET['id'];
$qry=mysql_query("SELECT * FROM `hospital_healthcenter` WHERE `hospital_id`='$cid'");
...
<form id="form1" name="form1" method="post" action="">
  <center><p><table width="200" border="0">
    ...
    <tr>
      ...
      <input type="text" name="textfield2" id="textfield2" value="<?php echo $row['place']?>"/></td>
    ...
    <tr>
      <th colspan="2" scope="row"><input type="submit" name="button" id="button" value="UPDATE" onclick="return validate()"/></th>
    </tr>
  </table></p></center>
...
if(isset($_POST['button']))
{
	...
	mysql_query("INSERT INTO `login`(`username`,`password`,`type`)VALUES('$email','$password','$type')");
	$lid=mysql_insert_id();
	$qry=mysql_query("UPDATE `hospital_healthcenter` SET `name`='$name',`place`='$place',`district`='$district',`post`='$post',`pin`='$pincode',`phone`='$phone' WHERE `hospital_id`='$cid'");
	if($qry>0)
	{
		?>
        <script>
